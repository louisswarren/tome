...
rename {Γ} {Λ x α} {Λ .x α′} (Λ ap) d = close (dm⊢ d) (λ x z → z (λ z₁ → z₁ (λ z₂ → z₂))) (arrowelim (arrowintro (Λ x α) (univintro x (all⟨ Λ∣ x α ⟩) (rename ap (univelim (varterm x) (ident α x) (assume (Λ x α)))))) d)
...
rename {Γ} {V x α} {V .x β} (V ap) d = close (dm⊢ d) (λ x z z₁ → z z₁ (λ z₂ → z₂ (λ z₃ → z₃))) (existelim (all⟨ V∣ x β ⟩ all∪ (α all~ (all- List.[ refl ]))) d (existintro (varterm x) x (ident β x) (rename ap (assume α))))
