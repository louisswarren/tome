...
\end{code}
The following constructor exists primarily to `normalise' $\Gamma$, for example
replacing a proof of $\{\alpha\} - \alpha \vdash \beta$ with a proof of
$\emptyset \vdash \beta$. It is also necessary for weakening results, for
example from $\Gamma \vdash \alpha$ to $\Gamma, \beta \vdash \alpha$. While
this is not one of the usual deduction rules, it will need to be used only at
the beginning of a proof to finalise the ensemble of assumptions. We require
...
either of variable freedom or variable substitution. The propositions proved
here have been formulated so that Agda's built-in proof search should be able
to supply them.
\begin{code}
